    ...
    $.fn.tablemanager = function (options = null) {
        ...
        **/
        var Table   = $(this),
            ...
            cells,
            clen;
            // sheetButton = document.querySelectorAll(".sheet-btn");
        ...
        if (options !== null) {
            ...
            **/
            ...
            if (
                ...
                // div num rows
                var numrowsDiv =
                    '<div id="for_numrows" class="for_numrows"><label for="numrows">' +
            ...
            **/
            if (pagination === true || Table.hasClass("tablePagination")) {
                ...
                var pagesDiv =
                    '<div class="pagination-div"><p style="margin:0;">Page<p style="width:18px;background:red;"></p><p id="s-n-pg" style="margin:0;">of</p><div id="pagesControllers" class="pagesControllers"></div></div>';
...
var sums=[]; //to store sums of all columns 
var sum=0;   //to store sum of one column



    for(var i=3,j=0;i<14;i++,j++)
    {
        sum=0;
        $(rows).each(function()
        {
        var value = $(this).find("td:nth-child("+i+")").text(); //fetching data from tds
        ...
        sums.push(sum);//pushing the sum of column into array
    }
...
for(var i=3,j=0;i<14;i++,j++)
    {
        if(sums[j] !== 0)
        {
            $("tfoot td:nth-child("+i+")").text(sums[j]); //adding sums to respective tds
        }
        else
        {
            $("tfoot td:nth-child("+i+")").text("");
        }    
    }      
...
//end of sum script
        ...
        perPage = (can be null) number of Results per page
        **/
